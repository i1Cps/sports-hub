...
# Environment to train the Quadruped agent to long jump
class QuadrupedLongJump(PipelineEnv):
    def __init__(self, sys=None):
        # Grab xml model of the scene and load into mujoco
        scene_path = "sports_hub/environments/longjump_quadruped/model.xml"
        mj_model = mujoco.MjModel.from_xml_path(scene_path)
        if not sys: sys = mjcf.load_model(mj_model)
        ...
        self._reset_noise_scale = 0.10
        self._agent_healthy_height = 0.2
        self._agent_healthy_torso_angle = 0.1
        ...
        # Geoms
        self._sand_pit_geom_index = mj_model.geom("sand").id
        self._floor_geom_index = mj_model.geom("floor").id

        agent_collision_geom_names = [
    ...
    def _compute_termination(self, data: mjx.Data) -> jax.Array:
        ...
        agent_collisions_with_floor = check_agent_geom_collisions(self._floor_geom_index)
        agent_collisions_with_sand = check_agent_geom_collisions(self._sand_pit_geom_index)
        agent_collisions = jp.logical_or(agent_collisions_with_floor,agent_collisions_with_sand)

        agent_geom_x_positions = data.geom_xpos[self._agent_collision_geom_idxs,0]
        agent_collision_distances = jp.where(agent_collisions,agent_geom_x_positions,-jp.inf) 
    ...
    def _compute_reward(self, data0: mjx.Data, data: mjx.Data, info) ->jp.ndarray:
        ...
        # Calculate current and previous pose of humanoid
        ...
        closer_to_target_reward = change_in_distance * self._agent_closer_to_target_reward_weight
        ...
        agent_collision_distances = jp.where(agent_collisions,agent_geom_x_positions - self._jumpline_pos[0], -jp.inf) 
        ...
        jump_distance = jp.where(landed_over_jumpline,jp.max(agent_collision_distances,initial=0.0),0.0)
        jump_length_reward = jump_distance * self._agent_jump_length_reward_weight

        reward =  closer_to_target_reward + velocity_reward + jump_height_reward + jump_length_reward
    ...
    def _get_obs(self, data: mjx.Data) -> jp.ndarray:
        ...
        # Get global pose of agent
        agent_pose = data.site_xpos[self._agent_site_index]
...
class QuadrupedLongJump_Eval(QuadrupedLongJump):
    def __init__(self):
        super().__init__()
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        reward = self._compute_reward(data0=data0, data=data, info=state.info)
        state.info["jump_complete"] = jp.logical_or(state.info["jump_complete"],reward > 0.0)
        done = 0.0
        obs = self._get_obs(data)
